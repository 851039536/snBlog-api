...
namespace Snblog.Service.Service
{
    public class SnippetTagService : BaseService, ISnippetTagService
    {
        ...
        public async Task<List<SnippetTagDto>> GetPagingAsync(int pageIndex, int pageSize, bool isDesc, bool cache)
        {
            ...
            if (isDesc) {
               rDto.eList = _mapper.Map<List<SnippetTagDto>>(await _service.SnippetTags.OrderByDescending(c => c.Id).Skip((pageIndex - 1) * pageSize).Take(pageSize).ToListAsync());
                } else {
               rDto.eList = _mapper.Map<List<SnippetTagDto>>(await _service.SnippetTags.OrderBy(c => c.Id).Skip((pageIndex - 1) * pageSize).Take(pageSize).ToListAsync());
                }
        ...
        public async Task<List<SnippetTagDto>> GetAllAsync(bool cache)
        {
            ...
            rDto.eList = _mapper.Map<List<SnippetTagDto>>(await _service.SnippetTags.AsNoTracking().ToListAsync());
